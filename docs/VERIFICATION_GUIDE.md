...
### Problem: Partial trace data (missing fields)

**Error:**
```
⚠ Trace missing required field: tool
```

**Cause:** The trace was generated by an older version of the logger that didn't record all RFC 0.1.0 fields.

**Solution:**
1. Check trace version: `jq '.version' .agent-trace/<commit>.json`
2. If version is `"1.0"` but fields are missing, the trace was likely backfilled
3. Re-generate the trace: `python backfill_traces.py --commit <sha>`
4. Legacy traces (pre-RFC) are expected to lack `tool` and `vcs` fields

### Problem: Batch verification timeout

**Error:**
```
Batch verification taking too long (>60s for 50 commits)
```

**Cause:** Large commit ranges with many trace files cause sequential I/O bottleneck.

**Solution:**
1. Use the Python verifier for better performance: `python scripts/verify-batch.py HEAD~50..HEAD`
2. Narrow the commit range
3. Use `--json` to export results and review offline

### Problem: Windows line endings affecting hash

**Error:**
```
✗ SHA-256 hash mismatch (Windows CRLF issue)
```

**Cause:** `git checkout` on Windows may convert LF to CRLF in trace JSON files.

**Solution:**
1. Add to `.gitattributes`: `*.json text eol=lf`
2. Re-checkout trace files: `git checkout -- .agent-trace/`
3. Alternatively, run verification in WSL or Git Bash

### Problem: Python verify script errors
...
**Cause:** Python version too old (< 3.7).

**Solution:**
1. Check Python version: `python3 --version`
2. Ensure Python >= 3.7 is installed
3. Use the bash scripts as fallback: `bash scripts/verify-trace.sh <commit>`
